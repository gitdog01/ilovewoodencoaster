...
4. 그다음이 4단계(학습 코드, model/ 아래 tokenizer.py만 있는 상태).
...
4단계 들어가기 전에 결정할 것: (a) 이대로 학습하거나, (b) 뱅크 쪽 영역을 더
모으거나. (b)는 A\*를 손댈 필요가 없다 — `banked=True` 일 때 맨턴 폴백을 아예
빼고 실패 시 재설계하게 하면 된다. 설계는 오프라인이라 게임 왕복이 없어서
성공률이 떨어져도 CPU만 더 쓴다.
